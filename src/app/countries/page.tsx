  ...
  return (
    <>
      ...
      <main className="px-4 py-12 sm:px-6 lg:px-8">
        {/* Same 90rem measure as the other listings; the grid still runs five
            across on desktop, so each card is simply bigger. */}
            ...
            <div className="space-y-12">
              {continents.map((continent) => (
                <section key={continent.id} aria-labelledby={`continent-${continent.slug}`}>
                  ...
                  <ul className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5 lg:gap-6">
                      ...
                      return (
                        <li key={country.id}>
                          <Link
                            ...
                            className="group flex h-full flex-col items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 p-5 text-center lg:gap-4 lg:p-7 shadow-sm transition-colors hover:border-emerald-300"
                          >
                            {flag ? (
                              ...
                              <Image
                                ...
                                height={80}
                                unoptimized
                                className="h-16 w-16 rounded-full object-cover shadow-sm lg:h-20 lg:w-20"
                              />
                            ) : (
                              <span
                                aria-hidden="true"
                                className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-400 lg:h-20 lg:w-20"
                              >
                                {country.code ?? '—'}
                              </span>
                            )}

                            <span className="font-semibold text-slate-800 group-hover:text-emerald-700 lg:text-lg">
                              {country.name}
                            </span>
                            ...
                            <span className="mt-auto w-full rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-white shadow-sm shadow-emerald-500/25 lg:py-2.5 lg:text-sm">
